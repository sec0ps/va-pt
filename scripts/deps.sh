...
sudo apt-get install -y ike-scan nfs-kernel-server httping ptunnel libaspell-dev autoconf libpcap-dev
...
sudo apt-get install -y samba libpam-smbpass libevent-dev flex bison libgeoip-dev chntpw openconnect
