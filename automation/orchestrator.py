...
class Orchestrator:
    ...
    def _do_analyze(self, ip):
        ...
        matched by CVE (from the -sV/vulners scan) and product name, plus the curated
        unauthenticated tier. A host with no avenue goes straight to CLEAN; otherwise
        it enters ATTACKING."""
        host = self.run.host_copy(ip)
        candidates = []
        for svc in host.services:
...
def _parse_args(argv):
    ...
    p.add_argument("--nse-catalog", default="",
                   help="path to a prebuilt NSE detection catalog for the verify "
                        "pass; empty uses the engine default location")
...
def main(argv=None):
    ...
    ocfg = OrchestratorConfig(
        ...
        verify_catalog_path=args.nse_catalog or "",
